...
class JFUSEConfig(BaseModel):
    ...
    # =========================================================================
    model_config_name: Literal[
        "prms", "prms_gradient", "max_gradient", "sacramento", "topmodel", "vic"
    ] = Field(
    ...
    enable_snow: bool = Field(
        default=True, description="Enable snow accumulation and melt processes."
    )
    ...
    spatial_mode: Literal["lumped", "distributed", "auto"] = Field(
        default="auto", description="Spatial modeling mode. 'auto' infers from domain definition."
    )

    n_hrus: int = Field(
        default=1, ge=1, description="Number of Hydrological Response Units (for distributed mode)."
    )

    enable_routing: bool = Field(
        default=False, description="Enable Muskingum-Cunge routing for distributed mode."
    )
    ...
    warmup_days: int = Field(
        default=365, ge=0, description="Number of warmup days for model spinup."
    )

    timestep_days: float = Field(
        default=1.0, ge=0.01, le=1.0, description="Model timestep in days. Default is 1.0 (daily)."
    )

    jit_compile: bool = Field(
        default=True, description="JIT compile model for faster execution (JAX)."
    )

    use_gpu: bool = Field(
        default=False, description="Use GPU acceleration (requires JAX with CUDA support)."
    )
    ...
    calibration_metric: Literal["KGE", "NSE"] = Field(
        default="KGE", description="Objective function for calibration."
    )
    ...
    forcing_file: Optional[str] = Field(
        default=None, description="Path to forcing data file (NetCDF or CSV)."
    )
    ...
    # =========================================================================
    initial_s1: float = Field(default=0.0, ge=0.0, description="Initial upper storage (mm).")

    initial_s2: float = Field(default=50.0, ge=0.0, description="Initial lower storage (mm).")

    initial_snow: float = Field(default=0.0, ge=0.0, description="Initial snow storage (mm SWE).")

    # =========================================================================
    # Routing Configuration (for distributed mode)
    # =========================================================================
    routing_substep_method: Literal["fixed", "adaptive"] = Field(
        default="adaptive", description="Sub-stepping method for Muskingum-Cunge routing stability."
    )
    ...
    save_states: bool = Field(
        default=False, description="Save internal state variables (storages, etc.) to output."
    )
